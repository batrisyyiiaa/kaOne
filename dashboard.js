...
// ── AI CHAT ──────────────────────────────────────────────────
const MOCK_AI = [
  'Based on your income of RM 3,200/month, save RM 576/month for LHDN. Your effective rate is ~18%.',
  'If you cancel Netflix (RM 54/month) you save RM 648/year — it has been unused for 30 days!',
  'GrabFood spending is up 40% this month. Setting a RM 120/month limit would save RM 60.',
  'Shopee purchases qualify as business deductions — marking them could save RM 1,440/year in tax.',
  'Your financial health score is 72/100. Main risk: unused subscriptions + low emergency fund.',
];
let mockIdx = 0;
...
async function sendChat() {
  ...
  try {
    const apiKeyToUse = currentApiKey || document.getElementById('api-key-input')?.value || '';
    if (!apiKeyToUse) throw new Error('No API key');
    ...
    const res = await fetch('https://api.ilmu.ai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'ILMU-GLM-5.1',
        messages: [
          { role:'system', content:'You are kaOne AI Analyst for Malaysian SME owners. Answer financial questions concisely using RM, LHDN, PCB context. Max 3 sentences.' },
    ...
    const reply = data.choices?.[0]?.message?.content || MOCK_AI[mockIdx++ % MOCK_AI.length];
    document.getElementById('chat-typing').style.display = 'none';
    addChatMessage('ai', reply);
    await dbAdd('chatHistory', { role:'ai', content: reply });
  } catch(e) {
    document.getElementById('chat-typing').style.display = 'none';
    const fallback = MOCK_AI[mockIdx++ % MOCK_AI.length];
