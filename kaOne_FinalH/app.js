const GEMINI_API_KEY = "AIzaSyBtRFJwyah5C5dohLCp92OdMYmGQVnxq34"; // leave empty, user will input OR set your key here
...
async function sendChat() {
  ...
  const apiKey = document.getElementById('api-key-input')?.value || GEMINI_API_KEY;
  
  try {
    const response = await callGemini(msg, apiKey);
    showTyping(false);
...
function testApiKey() {
  ...
  setTimeout(() => {
    if (key && key.startsWith('AIza')) {
      ...
      status.textContent = '● API Connected to GEMINI';
